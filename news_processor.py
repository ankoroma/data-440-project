...
class NewsProcessor:
    @staticmethod
    def clean_and_extract_keywords(news: dict) -> dict:
        """
        Cleans the news article title and content, extracts keywords, and updates the news dictionary.

        Args:
            news (dict): A dictionary containing the news article information.

        Returns:
            dict: The updated news dictionary with cleaned title, cleaned content, and extracted keywords.
        """
        ...
        df, df_chunk, kwords_dict = KeywordsExtract.extract_keywords(
    ...
    @staticmethod
    def extract_keywords_news_list(news_list: list) -> list:
        """
        Extracts keywords from a list of news articles.

        Args:
            news_list (list): A list of news articles.

        Returns:
            list: A list of processed news articles with extracted keywords.
        """
        processed_news = [
            NewsProcessor.clean_and_extract_keywords(a) for a in news_list
        ]
        return processed_news

    @staticmethod
    def load_news_csv(file: str) -> dict:
        """
        Load news data from a CSV file and process it into a dictionary.

        Args:
            file (str): The path to the CSV file.

        Returns:
            dict: A dictionary containing the processed news data.
        ...
        _df["pubDate"] = _df["pubDate"].apply(
            lambda x: dt.datetime.strftime(x, "%Y-%m-%dT%H:%M:%S")
        )
        _dict = _df.to_dict(orient="records")
        return _dict

    @staticmethod
    def get_provider_names(articles: list) -> list:
        """
        Get a list of unique provider names from a list of articles.

        Args:
            articles (list): A list of articles.

        Returns:
            list: A list of unique provider names.
        """
        _providers = [
            d["providerName"] if "providerName" in d else d["provider"]
            for d in articles
        ]
