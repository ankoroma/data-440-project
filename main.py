...
if __name__ == "__main__":
    ...
    news_articles = JsonLoader.load_json(
        "data/reuters_cleaned_with_keywords_post_processed.json"
    )
    for _d in news_articles:
        _d["pubDate"] = dt.datetime.strptime(_d["pubDate"], "%Y-%m-%dT%H:%M:%S")
    # return news articles published after Nov 15, 2019
    news_articles = [
        _d for _d in news_articles if _d["pubDate"] > dt.datetime(2019, 11, 15)
    ]
    news_clusters = NewsClustering.cluster_news_by_weighted_keywords(
        news_articles, eps=0.35, max_size=500
    )
