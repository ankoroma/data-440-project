...
class KeywordsExtract:
    ...
    months = [cd.month_name[i] for i in range(1, 13)] + [
        cd.month_abbr[i] for i in range(1, 13)
    ]
    lookups = Lookups()
    lemma_keep = ["data"]
    ...
    def extract_keywords(
        ...
        title: str,
        content: str,
        title_entity_weight: int = 4,
        title_noun_chunk_weight: int = 2,
    ) -> tuple:
        """
        ...
        df_ent_grp, _ = cls.extract_entities(
            ner_doc, title=title_txt, title_weight=title_entity_weight
        )
        df_chunk_grp, _ = cls.extract_noun_chunks(
            ner_doc, title=title_txt, title_weight=title_noun_chunk_weight
        )
        df_keywords = cls.filter_keywords_by_score(
            df_ent_grp, score_threshold=0.05, max_ents=10
        )
        df_chunks = cls.filter_keywords_by_score(
            df_chunk_grp, score_threshold=0.05, max_ents=10
        )
        if not df_chunks.empty:
            df_keywords = df_keywords.append(df_chunks, ignore_index=True, sort=False)
            df_keywords.sort_values(
                by=["total_score", "start"], inplace=True, ascending=[False, True]
            )
            df_keywords.drop_duplicates(subset=["entity"], inplace=True)
            df_keywords.drop_duplicates(subset=["lemma"], inplace=True)
        keywords = [
            {"keyword": str(r["entity"]), "weight": int(r["total_score"])}
    ...
    def extract_entities(
        cls, ner_document, title: str = "", title_weight: int = 4
    ) -> tuple:
        """
        ...
        if not df.empty:
            ...
            df["weight"] = df["start"].apply(
                lambda x: title_weight if (x < len(title)) else 1
            )
    ...
    def extract_noun_chunks(
        cls, ner_document, title: str = "", title_weight: int = 2
    ) -> tuple:
        """
        ...
        """
        chunks = [
            ch for ch in list(ner_document.noun_chunks) if (ch.root.ent_type_ == "")
        ]
        ...
        chunks = [ch for ch in chunks if len(ch) > 0]
        chunks_list = [
            [cls.lemma_last_word(ch), ch.start_char, ch.end_char, ch.label_, ch.lemma_]
            for ch in chunks
        ]
        cols = ["entity", "start", "end", "label", "lemma"]
    ...
    @classmethod
    def filter_keywords_and_calculate_weight(cls, df) -> tuple:
        """
        ...
        if not df.empty:
            ...
            df = df[
                df["entity"].apply(
                    lambda x: re.search("\s+\d+|".join(_months) + "\s+\d+", str(x))
                    is None
                )
            ]
            if df.empty:
        ...
        df_merge.sort_values(
            by=["weight", "start"], inplace=True, ascending=[False, True]
        )
    ...
    def filter_keywords(cls, df):
        ...
        if not df.empty:
            ...
            df["entity"] = df["entity"].apply(
                lambda x: re.sub("\.|[\-'\$\/\\\*\+\|\^\#\@\~\`]{2,}", "", str(x))
            )
            df = df[
                df["entity"].apply(
                    lambda x: re.search('\(|\)|\[|\]|"|\:|\{|\}|\^|\*|\;|\~|\|', str(x))
                    is None
                )
            ]
    ...
    def trim_tags(
        ...
        if len(s) < 1:
            return s
        s1 = s
        if (for_type == "chunk") and (
            re.search("|".join(punctuation), s1.text) is not None
        ):
    ...
    def filter_keywords_by_score(
        df, score_threshold: float = 0.1, max_ents: int = 50, top_n: int = 5
    ):
        """
    ...
    @staticmethod
    def merge_keywords_by_similarity(df, th: float = 0.4):
        """
        ...
        if len(df) > 1:
            ...
            most_sim = [
                [j for j in range(len(m)) if m[i, j] >= th]
            ...
            df.loc[df["total_score"] == 0.0, "total_score"] = (
                df.loc[df["total_score"] == 0.0, "weight"] * 1.0
            )
