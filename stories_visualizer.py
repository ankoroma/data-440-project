...
def plot_trending_news(news_by_period: list) -> None:
    """
    Plots the trending news based on the given news_by_period data.

    Parameters:
    news_by_period (list): A list of dictionaries representing news articles grouped by period.
    ...
    for period in stories:
        for i, story in enumerate(period):
            ...
            period[i] = _text
    stories_size = [
        [st["cluster_size"] for st in news[:top_stories]] for news in news_by_period
    ]
    df_topics = pd.DataFrame(
        stories, columns=["topic" + str(st) for st in range(top_stories)]
    )
    stories_size = np.log2(stories_size) + 0.3
    df_topics_size = pd.DataFrame(
        stories_size, columns=["size" + str(st) for st in range(top_stories)]
    )
    pos_y = [0.2 * (ith % 4) - 0.3 for ith, _ in enumerate(stories_size)]
    pos = [
        [(-1) ** ith * (1.3 + jth * 1.2) + pos_y[ith] for jth, size in enumerate(day)]
        for ith, day in enumerate(stories_size)
    ]
    ...
    for i in range(top_stories):
        ax.scatter(
            x,
            df_pos["pos" + str(i)],
            s=df_topics_size["size" + str(i)] * 1200,
            facecolors="lightsalmon",
            alpha=0.4,
            edgecolors="black",
            linewidth=2,
        )
    ...
    ax.set_title("Trending Stories (Nov 2019 - July 2020)", fontsize=12, color="black")
    ...
    hw, hl = 1.0 / 40.0 * (ymax - ymin), 1.0 / 40.0 * (
        xmax - xmin
    ...
    # plt.savefig("viz/trending_stories.png", dpi=200)
...
def visualize_trending_stories(processed_news: list) -> None:
    """
    Visualizes trending stories based on processed news data.

    Args:
        processed_news (list): A list of dictionaries representing processed news data.
    ...
    for p_i in range(periods, -1, -1):
        ...
        _news_in = [
            _d for _d in processed_news if _start_time < _d["pubDate"] <= _end_time
        ]
        if not _news_in:
            print("no news between {} and {}".format(_start_time, _end_time))
            continue
        _data = [
            [i, _d["cluster_id"], _d["kwords"], 1] for i, _d in enumerate(_news_in)
        ]
        _df = pd.DataFrame(_data, columns=["i", "cluster_id", "kwords", "cluster_size"])
        _df_g = _df.groupby(by="cluster_id").agg(
            {"cluster_size": sum, "kwords": lambda x: list(x)[0]}
        )
